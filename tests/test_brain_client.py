...
def test_chat_brain_in_process(settings):
    with patch("cluny.supervisor.run_chat") as mock_chat:
        mock_chat.return_value = SupervisorResult(route="ask", answer="hi", tool_calls=[])
...
def test_chat_brain_http(monkeypatch, tmp_path):
    ...
    with patch("cluny.brain_client.BrainClient.chat") as mock_http:
        mock_http.return_value = SupervisorResult(route="ask", answer="via http", tool_calls=[])
        result = chat_brain("hello", settings=settings)
